...
export interface RecommendedRouteStop {
  ...
  // 아래는 지도 핀 위 장소 정보 태그에 보여 줄 값입니다. 목 데이터에는 일부
  // 코스에만 있어 optional 입니다.

  /** 도로명 주소. 없으면 지번 주소, 둘 다 없으면 null 입니다. */
  address?: string | null;
  /** 머무는 시간(분) */
   ...
   *
   * 서버 필드명(`pathCoordinates`)만 봐서는 방향을 알 수 없어(첫 경유지는 항상
   * 빈 배열, 마지막 경유지는 값이 있음) 헷갈리지 않도록 이름에 방향을 박아
   * 둡니다.
   *
